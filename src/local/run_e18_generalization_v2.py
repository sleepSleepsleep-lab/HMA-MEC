...
def main():
    ...
    _only = os.environ.get("E18V2_ONLY")
    if _only:
        variants = [v for v in variants if v == _only]
        global OUT_JSON
        OUT_JSON = OUT_JSON.replace(".json", f"_{_only}.json")
        _rec = Recorder(f"e18v2_{_only}")
    out = {}
    _rec = Recorder("e18")
            ...
            for ep in range(N_EPISODES):
                ...
                for name in rows:
                    ...
                    env = GenEnv(variant, num_users=K,
                                 num_servers=num_servers, seed=SEED + sd * 100 + ep)  # v2: 非碰撞
                    if name == "HMA-Distill":
